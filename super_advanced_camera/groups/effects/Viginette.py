...
def create_viginette_group() -> NodeTree:
    ...
    # Create the nodes
    roundness_map_range_node = sac_viginette_group.nodes.new("ShaderNodeMapRange")
    ...
    intensity_map_range_node = sac_viginette_group.nodes.new("ShaderNodeMapRange")
    ...
    midpoint_map_range_node = sac_viginette_group.nodes.new("ShaderNodeMapRange")
    ...
    midpoint_add_node = sac_viginette_group.nodes.new("ShaderNodeMath")
    ...
    whiten_node = sac_viginette_group.nodes.new("ShaderNodeMath")
    ...
    scale_node.inputs["Type"].default_value = "Relative"

    multiply_node = sac_viginette_group.nodes.new("ShaderNodeMath")
    multiply_node.operation = "MULTIPLY"

    pingpong_node = sac_viginette_group.nodes.new("ShaderNodeMath")
    pingpong_node.operation = "PINGPONG"
    pingpong_node.inputs[1].default_value = 0.5

    multiply_node2 = sac_viginette_group.nodes.new("ShaderNodeMath")
    multiply_node2.operation = "MULTIPLY"
    multiply_node2.inputs[1].default_value = 2
    multiply_node2.use_clamp = True

    greater_than_node = sac_viginette_group.nodes.new("ShaderNodeMath")
    greater_than_node.operation = "GREATER_THAN"
    greater_than_node.inputs[1].default_value = 0.5
    greater_than_node.use_clamp = True

    add_node = sac_viginette_group.nodes.new("ShaderNodeMath")
    add_node.operation = "ADD"
    add_node.use_clamp = True

    directional_blur_node = sac_viginette_group.nodes.new("CompositorNodeDBlur")
    directional_blur_node.inputs["Samples"].default_value = 6
    directional_blur_node.inputs["Scale"].default_value = 0.25
    directional_blur_node.name = "SAC Effects_Viginette_Directional_Blur"

    color_mix_node = sac_viginette_group.nodes.new("ShaderNodeMix")
    ...
    link_nodes(sac_viginette_group, roundness_map_range_node, 0, lense_distortion_node, "Distortion")
    ...
    link_nodes(sac_viginette_group, midpoint_add_node, 0, scale_node, "X")
    link_nodes(sac_viginette_group, midpoint_add_node, 0, scale_node, "Y")
    ...
    link_nodes(sac_viginette_group, greater_than_node, 0, color_mix_node, 6)
    link_nodes(sac_viginette_group, input_node, 0, color_mix_node, 7)
    link_nodes(sac_viginette_group, color_mix_node, 2, output_node, 0)
