...
def create_filmgrain_group() -> NodeTree:
    ...
    # Create the nodes
    add_node = sac_filmgrain_group.nodes.new("ShaderNodeMath")
    ...
    lens_distortion_node.inputs["Distortion"].default_value = 0.00001

    rgb_to_bw_node = sac_filmgrain_group.nodes.new("CompositorNodeRGBToBW")

    math_subtract_node = sac_filmgrain_group.nodes.new("ShaderNodeMath")
    ...
    bilateral_blur_node.inputs["Threshold"].default_value = 0.4
    bilateral_blur_node.inputs["Size"].default_value = 3
    bilateral_blur_node.name = "SAC Effects_FilmGrain_Blur"

    color_screen_node = sac_filmgrain_group.nodes.new("ShaderNodeMix")
    color_screen_node.data_type = "RGBA"
    ...
    link_nodes(sac_filmgrain_group, bilateral_blur_node, 0, color_screen_node, 7)
    link_nodes(sac_filmgrain_group, input_node, 0, color_screen_node, 6)
    link_nodes(sac_filmgrain_group, color_screen_node, 2, output_node, 0)
