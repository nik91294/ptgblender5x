...
def create_infrared_group() -> NodeTree:
    ...
    # Create the nodes
    add_node = sac_infrared_group.nodes.new("ShaderNodeMath")
    add_node.operation = "ADD"
    add_node.inputs[1].default_value = 0
    add_node.name = "SAC Effects_Infrared_Add"

    color_ramp_node = sac_infrared_group.nodes.new("ShaderNodeValToRGB")
    ...
    color_mix_node = sac_infrared_group.nodes.new("ShaderNodeMix")
    ...
    link_nodes(sac_infrared_group, color_ramp_node, 0, color_mix_node, 7)
    link_nodes(sac_infrared_group, input_node, 0, color_mix_node, 6)
    link_nodes(sac_infrared_group, color_mix_node, 2, output_node, 0)
