...
def create_iso_group() -> NodeTree:
    ...
    # Create the nodes
    add_node = sac_iso_group.nodes.new("ShaderNodeMath")
    ...
    lens_distortion_node.inputs["Jitter"].default_value = True
    lens_distortion_node.inputs["Distortion"].default_value = 0.000001

    color_subtract_node = sac_iso_group.nodes.new("ShaderNodeMix")
    color_subtract_node.data_type = "RGBA"
    color_subtract_node.blend_type = "SUBTRACT"
    color_subtract_node.clamp_result = True

    math_less_than_node = sac_iso_group.nodes.new("ShaderNodeMath")
    math_less_than_node.operation = "LESS_THAN"
    math_less_than_node.inputs[1].default_value = 0.03

    color_mix_node = sac_iso_group.nodes.new("ShaderNodeMix")
    color_mix_node.data_type = "RGBA"
    color_mix_node.blend_type = "MIX"
    color_mix_node.inputs[7].default_value = (0, 0.2, 0, 1)

    despeckle_node = sac_iso_group.nodes.new("CompositorNodeDespeckle")
    despeckle_node.inputs["Color Threshold"].default_value = 0
    despeckle_node.inputs["Neighbor Threshold"].default_value = 0
    despeckle_node.name = "SAC Effects_ISO_Despeckle"

    color_add_node = sac_iso_group.nodes.new("ShaderNodeMix")
    color_add_node.data_type = "RGBA"
    ...
    link_nodes(sac_iso_group, lens_distortion_node, 0, color_subtract_node, 6)
    link_nodes(sac_iso_group, color_subtract_node, 2, math_less_than_node, 0)
    link_nodes(sac_iso_group, math_less_than_node, 0, color_mix_node, 0)
    link_nodes(sac_iso_group, color_subtract_node, 2, color_mix_node, 6)
    link_nodes(sac_iso_group, color_mix_node, 2, despeckle_node, 1)
    link_nodes(sac_iso_group, despeckle_node, 0, color_add_node, 7)
    link_nodes(sac_iso_group, input_node, 0, color_add_node, 6)
    link_nodes(sac_iso_group, color_add_node, 2, output_node, 0)
