...
def create_curves_group() -> NodeTree:
    ...
    rgb_curves_node.inputs["Factor"].default_value = 0

    hsv_curves_node = sac_curves_group.nodes.new("CompositorNodeHueCorrect")
    hsv_curves_node.name = "SAC Colorgrade_Curves_HSV"
    hsv_curves_node.inputs["Factor"].default_value = 0

    # Create the links
    link_nodes(sac_curves_group, input_node, 0, rgb_curves_node, "Image")
    link_nodes(sac_curves_group, rgb_curves_node, 0, hsv_curves_node, "Image")
