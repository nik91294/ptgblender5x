...
def create_sharpen_group() -> NodeTree:
    ...
    # Create the nodes
    value_node = sac_sharpen_group.nodes.new("ShaderNodeValue")
    value_node.name = "SAC Colorgrade_Presets_Sharpen"
    value_node.outputs[0].default_value = 0

    map_range_node = sac_sharpen_group.nodes.new("ShaderNodeMapRange")
    ...
    soften_node.inputs["Type"].default_value = "Soften"

    sharpen_node = sac_sharpen_group.nodes.new("CompositorNodeFilter")
    sharpen_node.inputs["Type"].default_value = "Diamond Sharpen"

    mix_node = sac_sharpen_group.nodes.new("ShaderNodeMix")
    ...
    link_nodes(sac_sharpen_group, subtract_node_1, 0, soften_node, "Factor")
    link_nodes(sac_sharpen_group, subtract_node_2, 0, sharpen_node, "Factor")
    link_nodes(sac_sharpen_group, input_node, 0, soften_node, "Image")
    link_nodes(sac_sharpen_group, input_node, 0, sharpen_node, "Image")
    link_nodes(sac_sharpen_group, greater_than_node, 0, mix_node, 0)
    link_nodes(sac_sharpen_group, soften_node, 0, mix_node, 6)
    link_nodes(sac_sharpen_group, sharpen_node, 0, mix_node, 7)
    link_nodes(sac_sharpen_group, mix_node, 2, output_node, 0)
