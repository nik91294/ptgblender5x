...
def create_highlighttint_group() -> NodeTree:
    ...
    # Create the nodes
    add_node = sac_highlighttint_group.nodes.new("ShaderNodeMath")
    add_node.inputs[1].default_value = 0
    add_node.use_clamp = True

    mixrgb_node = sac_highlighttint_group.nodes.new("ShaderNodeMix")
    mixrgb_node.data_type = "RGBA"
    ...
    link_nodes(sac_highlighttint_group, input_node, 0, mixrgb_node, 6)
    link_nodes(sac_highlighttint_group, add_node, 0, mixrgb_node, 0)
    link_nodes(sac_highlighttint_group, mixrgb_node, 2, output_node, 0)
