...
def create_shadowtint_group() -> NodeTree:
    ...
    # Create the nodes
    subtract_node = sac_shadowtint_group.nodes.new("ShaderNodeMath")
    subtract_node.operation = "SUBTRACT"
    subtract_node.inputs[0].default_value = 1
    subtract_node.use_clamp = True

    mixrgb_node = sac_shadowtint_group.nodes.new("ShaderNodeMix")
    ...
    link_nodes(sac_shadowtint_group, input_node, 0, mixrgb_node, 6)
    link_nodes(sac_shadowtint_group, subtract_node, 0, mixrgb_node, 0)
    link_nodes(sac_shadowtint_group, mixrgb_node, 2, output_node, 0)
