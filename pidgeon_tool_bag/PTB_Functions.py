...
def get_scene_compositor_tree(scene):
    """Return the scene's compositor node tree, or None if it doesn't have one yet.

    Blender 5.0 removed Scene.node_tree in favor of Scene.compositing_node_group.
    """
...
def ensure_scene_compositor_tree(scene):
    """Return the scene's compositor node tree, creating and assigning a fresh one if needed.

    Blender 5.0 removed Scene.node_tree in favor of Scene.compositing_node_group, and
    scene.use_nodes no longer creates a default node tree.
    """
    if hasattr(scene, "compositing_node_group"):
        tree = scene.compositing_node_group
        if tree is None:
            tree = bpy.data.node_groups.new(name=f"{scene.name} Compositing", type="CompositorNodeTree")
...
def get_compositor_output_node(tree):
    """Return the compositor node tree's final output node.

    Blender 5.0 removed the dedicated Composite node; compositor node trees now use
    a regular Group Output node whose first (Color) input is the final render result.
    """
