...
from ..pidgeon_tool_bag.PTB_Functions import bcolors, get_scene_compositor_tree, ensure_scene_compositor_tree
...
def create_denoiser_skip_group() -> NodeTree:
    ...
    # create the nodes
    add_color_node = denoiser_skip_tree.nodes.new("ShaderNodeMix")
    add_color_node.data_type = "RGBA"
    add_color_node.blend_type = "ADD"
    multiply_color_node = denoiser_skip_tree.nodes.new("ShaderNodeMix")
    multiply_color_node.data_type = "RGBA"
    multiply_color_node.blend_type = "MULTIPLY"

    # link the nodes
    link_nodes(denoiser_skip_tree, input_node, "Direct", add_color_node, 6)
    link_nodes(denoiser_skip_tree, input_node, "Indirect", add_color_node, 7)
    link_nodes(denoiser_skip_tree, add_color_node, 2, multiply_color_node, 6)
    link_nodes(denoiser_skip_tree, input_node, "Color", multiply_color_node, 7)
    link_nodes(denoiser_skip_tree, multiply_color_node, 2, output_node, "Image")
...
def create_denoiser_super_group() -> NodeTree:
    ...
    denoise_node_direct.inputs["Prefilter"].default_value = "Accurate"
    denoise_node_direct.inputs["HDR"].default_value = True

    denoise_node_indirect = denoiser_super_tree.nodes.new("CompositorNodeDenoise")
    denoise_node_indirect.inputs["Prefilter"].default_value = "Accurate"
    denoise_node_indirect.inputs["HDR"].default_value = True

    add_color_node = denoiser_super_tree.nodes.new("ShaderNodeMix")
    add_color_node.data_type = "RGBA"
    add_color_node.blend_type = "ADD"

    multiply_color_node = denoiser_super_tree.nodes.new("ShaderNodeMix")
    multiply_color_node.data_type = "RGBA"
    multiply_color_node.blend_type = "MULTIPLY"

    # link the nodes
    link_nodes(denoiser_super_tree, input_node, "Direct", denoise_node_direct, "Image")
    link_nodes(denoiser_super_tree, input_node, "Denoising Normal", denoise_node_direct, "Normal")
    link_nodes(denoiser_super_tree, input_node, "Denoising Albedo", denoise_node_direct, "Albedo")
    link_nodes(denoiser_super_tree, input_node, "Indirect", denoise_node_indirect, "Image")
    link_nodes(denoiser_super_tree, input_node, "Denoising Normal", denoise_node_indirect, "Normal")
    link_nodes(denoiser_super_tree, input_node, "Denoising Albedo", denoise_node_indirect, "Albedo")
    link_nodes(denoiser_super_tree, denoise_node_direct, "Image", add_color_node, 6)
    link_nodes(denoiser_super_tree, denoise_node_indirect, "Image", add_color_node, 7)
    link_nodes(denoiser_super_tree, add_color_node, 2, multiply_color_node, 6)
    link_nodes(denoiser_super_tree, input_node, "Color", multiply_color_node, 7)
    link_nodes(denoiser_super_tree, multiply_color_node, 2, output_node, "Image")
    link_nodes(denoiser_super_tree, denoise_node_direct, "Image", output_node, "Direct")
    link_nodes(denoiser_super_tree, denoise_node_indirect, "Image", output_node, "Indirect")
...
def create_super_group() -> NodeTree:
    ...
    emission_denoiser_node.inputs["Prefilter"].default_value = "Accurate"
    emission_denoiser_node.inputs["HDR"].default_value = True
    ...
    environment_denoiser_node.inputs["Prefilter"].default_value = "Accurate"
    environment_denoiser_node.inputs["HDR"].default_value = True
    environment_denoiser_node.name = "Environment Denoiser"
    environment_denoiser_node.mute = not settings.environment

    add_color_node_1 = super_tree.nodes.new("ShaderNodeMix")
    add_color_node_1.data_type = "RGBA"
    add_color_node_1.blend_type = "ADD"
    add_color_node_2 = super_tree.nodes.new("ShaderNodeMix")
    add_color_node_2.data_type = "RGBA"
    add_color_node_2.blend_type = "ADD"
    add_color_node_3 = super_tree.nodes.new("ShaderNodeMix")
    add_color_node_3.data_type = "RGBA"
    add_color_node_3.blend_type = "ADD"
    add_color_node_4 = super_tree.nodes.new("ShaderNodeMix")
    add_color_node_4.data_type = "RGBA"
    add_color_node_4.blend_type = "ADD"
    add_color_node_5 = super_tree.nodes.new("ShaderNodeMix")
    add_color_node_5.data_type = "RGBA"
    ...
    link_nodes(super_tree, input_node, "Emit", emission_denoiser_node, "Image")
    link_nodes(super_tree, input_node, "Denoising Normal", emission_denoiser_node, "Normal")
    link_nodes(super_tree, input_node, "Denoising Albedo", emission_denoiser_node, "Albedo")
    link_nodes(super_tree, input_node, "Env", environment_denoiser_node, "Image")
    link_nodes(super_tree, input_node, "Denoising Normal", environment_denoiser_node, "Normal")
    link_nodes(super_tree, input_node, "Denoising Albedo", environment_denoiser_node, "Albedo")
    link_nodes(super_tree, diffuse_node_group, "Image", add_color_node_1, 6)
    link_nodes(super_tree, glossy_node_group, "Image", add_color_node_1, 7)
    link_nodes(super_tree, add_color_node_1, 2, add_color_node_2, 6)
    link_nodes(super_tree, transmission_node_group, "Image", add_color_node_2, 7)
    link_nodes(super_tree, add_color_node_2, 2, add_color_node_3, 6)
    link_nodes(super_tree, volume_node_group, "Image", add_color_node_3, 7)
    link_nodes(super_tree, add_color_node_3, 2, add_color_node_4, 6)
    link_nodes(super_tree, emission_denoiser_node, "Image", add_color_node_4, 7)
    link_nodes(super_tree, add_color_node_4, 2, add_color_node_5, 6)
    link_nodes(super_tree, environment_denoiser_node, "Image", add_color_node_5, 7)
    link_nodes(super_tree, add_color_node_5, 2, output_node, "Image")
    ...
    link_nodes(super_tree, emission_denoiser_node, "Image", output_node, "Emit")
    link_nodes(super_tree, environment_denoiser_node, "Image", output_node, "Env")
...
def create_denoiser_high_group() -> NodeTree:
    ...
    # create the nodes
    add_color_node = denoiser_high_tree.nodes.new("ShaderNodeMix")
    add_color_node.data_type = "RGBA"
    add_color_node.blend_type = "ADD"

    multiply_color_node = denoiser_high_tree.nodes.new("ShaderNodeMix")
    multiply_color_node.data_type = "RGBA"
    multiply_color_node.blend_type = "MULTIPLY"

    denoise_node = denoiser_high_tree.nodes.new("CompositorNodeDenoise")
    denoise_node.inputs["Prefilter"].default_value = "Fast"
    denoise_node.inputs["HDR"].default_value = True

    # link the nodes
    link_nodes(denoiser_high_tree, input_node, "Direct", add_color_node, 6)
    link_nodes(denoiser_high_tree, input_node, "Indirect", add_color_node, 7)
    link_nodes(denoiser_high_tree, add_color_node, 2, multiply_color_node, 6)
    link_nodes(denoiser_high_tree, input_node, "Color", multiply_color_node, 7)
    link_nodes(denoiser_high_tree, multiply_color_node, 2, denoise_node, "Image")
    link_nodes(denoiser_high_tree, input_node, "Denoising Normal", denoise_node, "Normal")
    link_nodes(denoiser_high_tree, input_node, "Denoising Albedo", denoise_node, "Albedo")
    link_nodes(denoiser_high_tree, denoise_node, "Image", output_node, "Image")
...
def create_high_group() -> NodeTree:
    ...
    emission_denoiser_node.inputs["Prefilter"].default_value = "Fast"
    emission_denoiser_node.inputs["HDR"].default_value = True
    emission_denoiser_node.name = "Emission Denoiser"

    environment_denoiser_node = high_tree.nodes.new("CompositorNodeDenoise")
    environment_denoiser_node.inputs["Prefilter"].default_value = "Fast"
    environment_denoiser_node.inputs["HDR"].default_value = True
    environment_denoiser_node.name = "Environment Denoiser"

    add_color_node_1 = high_tree.nodes.new("ShaderNodeMix")
    add_color_node_1.data_type = "RGBA"
    add_color_node_1.blend_type = "ADD"
    add_color_node_2 = high_tree.nodes.new("ShaderNodeMix")
    add_color_node_2.data_type = "RGBA"
    add_color_node_2.blend_type = "ADD"
    add_color_node_3 = high_tree.nodes.new("ShaderNodeMix")
    add_color_node_3.data_type = "RGBA"
    add_color_node_3.blend_type = "ADD"
    add_color_node_4 = high_tree.nodes.new("ShaderNodeMix")
    add_color_node_4.data_type = "RGBA"
    add_color_node_4.blend_type = "ADD"
    add_color_node_5 = high_tree.nodes.new("ShaderNodeMix")
    add_color_node_5.data_type = "RGBA"
    add_color_node_5.blend_type = "ADD"
    ...
    link_nodes(high_tree, input_node, "Emit", emission_denoiser_node, "Image")
    link_nodes(high_tree, input_node, "Denoising Normal", emission_denoiser_node, "Normal")
    link_nodes(high_tree, input_node, "Denoising Albedo", emission_denoiser_node, "Albedo")
    link_nodes(high_tree, input_node, "Env", environment_denoiser_node, "Image")
    link_nodes(high_tree, input_node, "Denoising Normal", environment_denoiser_node, "Normal")
    link_nodes(high_tree, input_node, "Denoising Albedo", environment_denoiser_node, "Albedo")
    link_nodes(high_tree, diffuse_denoiser_node, "Image", add_color_node_1, 6)
    link_nodes(high_tree, glossy_denoiser_node, "Image", add_color_node_1, 7)
    link_nodes(high_tree, add_color_node_1, 2, add_color_node_2, 6)
    link_nodes(high_tree, transmission_denoiser_node, "Image", add_color_node_2, 7)
    link_nodes(high_tree, add_color_node_2, 2, add_color_node_3, 6)
    link_nodes(high_tree, volume_denoiser_node, "Image", add_color_node_3, 7)
    link_nodes(high_tree, add_color_node_3, 2, add_color_node_4, 6)
    link_nodes(high_tree, emission_denoiser_node, "Image", add_color_node_4, 7)
    link_nodes(high_tree, add_color_node_4, 2, add_color_node_5, 6)
    link_nodes(high_tree, environment_denoiser_node, "Image", add_color_node_5, 7)
    link_nodes(high_tree, add_color_node_5, 2, output_node, "Image")
    ...
    link_nodes(high_tree, emission_denoiser_node, "Image", output_node, "Emit")
    link_nodes(high_tree, environment_denoiser_node, "Image", output_node, "Env")
...
def create_standard_group() -> NodeTree:
    ...
    # link the nodes
    link_nodes(standard_tree, input_node, "Image", denoise_node, "Image")
    link_nodes(standard_tree, input_node, "Denoising Normal", denoise_node, "Normal")
    link_nodes(standard_tree, input_node, "Denoising Albedo", denoise_node, "Albedo")
    link_nodes(standard_tree, denoise_node, "Image", output_node, "Image")
...
def create_temporal_albedo_node() -> NodeTree:
    ...
    # Create the nodes
    add_color_node_1 = temporal_albedo_tree.nodes.new("ShaderNodeMix")
    add_color_node_1.data_type = "RGBA"
    add_color_node_1.blend_type = "ADD"
    add_color_node_2 = temporal_albedo_tree.nodes.new("ShaderNodeMix")
    add_color_node_2.data_type = "RGBA"
    add_color_node_2.blend_type = "ADD"
    add_color_node_3 = temporal_albedo_tree.nodes.new("ShaderNodeMix")
    add_color_node_3.data_type = "RGBA"
    add_color_node_3.blend_type = "ADD"
    add_color_node_4 = temporal_albedo_tree.nodes.new("ShaderNodeMix")
    add_color_node_4.data_type = "RGBA"
    add_color_node_4.blend_type = "ADD"

    # Link the nodes
    link_nodes(temporal_albedo_tree, input_node, "DiffCol", add_color_node_1, 6)
    link_nodes(temporal_albedo_tree, input_node, "GlossCol", add_color_node_1, 7)
    link_nodes(temporal_albedo_tree, add_color_node_1, 2, add_color_node_2, 6)
    link_nodes(temporal_albedo_tree, input_node, "TransCol", add_color_node_2, 7)
    link_nodes(temporal_albedo_tree, add_color_node_2, 2, add_color_node_3, 6)
    link_nodes(temporal_albedo_tree, input_node, "Emit", add_color_node_3, 7)
    link_nodes(temporal_albedo_tree, add_color_node_3, 2, add_color_node_4, 6)
    link_nodes(temporal_albedo_tree, input_node, "Env", add_color_node_4, 7)
    link_nodes(temporal_albedo_tree, add_color_node_4, 2, output_node, "Temporal Albedo")
...
def create_super_image_denoiser_group() -> NodeTree:
    ...
    set_alpha_node_super.inputs["Type"].default_value = "Replace Alpha"
    set_alpha_node_high = super_image_denoiser_tree.nodes.new("CompositorNodeSetAlpha")
    set_alpha_node_high.inputs["Type"].default_value = "Replace Alpha"
...
def connect_render_layers_to_sid():
    ...
    tree = ensure_scene_compositor_tree(bpy.context.scene)
...
def create_temporal_output_node():
    ...
    for node in tree.nodes:
        if node.type == "GROUP" and node.node_tree.name.startswith(".Super Image Denoiser"):
            ...
            output_node.directory = os.path.join(settings.working_directory, "processing", str(index))
            output_node.file_name = settings.custom_name + "######"
            ...
            # link the node
            link_nodes(tree, node, settings.quality, output_node, 0)
            output_node.file_output_items.new("NodeSocketColor", "Vector")
            link_nodes(tree, render_layer_node, "Vector", output_node, "Vector")
            output_node.file_output_items.new("NodeSocketColor", "Depth")
            link_nodes(tree, render_layer_node, "Denoising Depth", output_node, "Depth")
            output_node.file_output_items.new("NodeSocketColor", "Temporal Albedo")
            link_nodes(tree, node, "Temporal Albedo", output_node, "Temporal Albedo")
...
def update_multilayer_exr(self, context):
    compositor = get_scene_compositor_tree(bpy.context.scene)
    ...
    if self.multilayer_exr:
        ...
        for node in compositor.nodes:
            if node.type == "GROUP" and node.node_tree.name == ".Super Image Denoiser":
                ...
                output_node.format.file_format = "OPEN_EXR_MULTILAYER"
                output_node.directory = self.multilayer_exr_path + str(index) + "/"
                output_node.file_name = "Image"
                link_nodes(compositor, node, self.quality, output_node, 0)
                if self.quality == "Super":
                    # add new slot
                    output_node.file_output_items.new("NodeSocketColor", "DiffDir")
                    link_nodes(compositor, node, "DiffDir", output_node, "DiffDir")
                    output_node.file_output_items.new("NodeSocketColor", "DiffInd")
                    link_nodes(compositor, node, "DiffInd", output_node, "DiffInd")
                    output_node.file_output_items.new("NodeSocketColor", "DiffCol")
                    link_nodes(compositor, node, "DiffCol", output_node, "DiffCol")
                    output_node.file_output_items.new("NodeSocketColor", "GlossDir")
                    link_nodes(compositor, node, "GlossDir", output_node, "GlossDir")
                    output_node.file_output_items.new("NodeSocketColor", "GlossInd")
                    link_nodes(compositor, node, "GlossInd", output_node, "GlossInd")
                    output_node.file_output_items.new("NodeSocketColor", "GlossCol")
                    link_nodes(compositor, node, "GlossCol", output_node, "GlossCol")
                    output_node.file_output_items.new("NodeSocketColor", "TransDir")
                    link_nodes(compositor, node, "TransDir", output_node, "TransDir")
                    output_node.file_output_items.new("NodeSocketColor", "TransInd")
                    link_nodes(compositor, node, "TransInd", output_node, "TransInd")
                    output_node.file_output_items.new("NodeSocketColor", "TransCol")
                    link_nodes(compositor, node, "TransCol", output_node, "TransCol")
                    output_node.file_output_items.new("NodeSocketColor", "VolumeDir")
                    link_nodes(compositor, node, "VolumeDir", output_node, "VolumeDir")
                    output_node.file_output_items.new("NodeSocketColor", "VolumeInd")
                    link_nodes(compositor, node, "VolumeInd", output_node, "VolumeInd")
                    output_node.file_output_items.new("NodeSocketColor", "Emit")
                    link_nodes(compositor, node, "Emit", output_node, "Emit")
                    output_node.file_output_items.new("NodeSocketColor", "Env")
                    link_nodes(compositor, node, "Env", output_node, "Env")
                elif self.quality == "High":
                    output_node.file_output_items.new("NodeSocketColor", "Diffuse")
                    link_nodes(compositor, node, "Diffuse", output_node, "Diffuse")
                    output_node.file_output_items.new("NodeSocketColor", "Glossy")
                    link_nodes(compositor, node, "Glossy", output_node, "Glossy")
                    output_node.file_output_items.new("NodeSocketColor", "Transmission")
                    link_nodes(compositor, node, "Transmission", output_node, "Transmission")
                    output_node.file_output_items.new("NodeSocketColor", "Volume")
...
def create_temporal_median(minimum: bool) -> NodeTree:
    ...
    median_r = temporal_median_tree.nodes.new("ShaderNodeMath")
    median_r.operation = min_max
    median_g = temporal_median_tree.nodes.new("ShaderNodeMath")
    median_g.operation = min_max
    median_b = temporal_median_tree.nodes.new("ShaderNodeMath")
    median_b.operation = min_max
    median_a = temporal_median_tree.nodes.new("ShaderNodeMath")
...
def create_temporal_crop() -> NodeTree:
    ...
    target_width = render.resolution_x * render.resolution_percentage / 100
    target_height = render.resolution_y * render.resolution_percentage / 100
    overscan_width = render.resolution_x * overscan_percentage / 100
    overscan_height = render.resolution_y * overscan_percentage / 100

    crop_node = crop_tree.nodes.new(type="CompositorNodeCrop")
    crop_node.inputs["X"].default_value = (overscan_width - target_width) / 2
    crop_node.inputs["Y"].default_value = (overscan_height - target_height) / 2
    crop_node.inputs["Width"].default_value = target_width
    crop_node.inputs["Height"].default_value = target_height

    scale_node = crop_tree.nodes.new(type="CompositorNodeScale")
    scale_node.inputs["Type"].default_value = "Absolute"
    scale_node.inputs["X"].default_value = target_width
    scale_node.inputs["Y"].default_value = target_height
...
def create_temporal_align() -> NodeTree:
    ...
    ta_difference = temporal_align_tree.nodes.new(type="ShaderNodeMix")
    ta_difference.data_type = "RGBA"
    ta_difference.blend_type = "DIFFERENCE"
    ta_difference.clamp_result = True

    ta_multiply = temporal_align_tree.nodes.new(type="ShaderNodeMath")
    ...
    ta_dialate.inputs["Type"].default_value = "Feather"
    ta_dialate.inputs["Size"].default_value = settings.ted_radius
    ta_dialate.inputs["Falloff"].default_value = "Sphere"

    ta_mix = temporal_align_tree.nodes.new(type="ShaderNodeMix")
    ta_mix.data_type = "RGBA"
    ...
    vector_blur.inputs["Samples"].default_value = settings.motion_blur_samples
    vector_blur.inputs["Shutter"].default_value = settings.motion_blur_shutter_speed
    # NOTE: speed_min/speed_max/use_curved (min/max speed clamping and curved
    # interpolation) no longer exist on CompositorNodeVecBlur in Blender 5.x
    # and have no replacement socket - this functionality is currently lost.
    ...
    link_nodes(temporal_align_tree, input_node, "Frame + 1", alpha_over, "Background")
    link_nodes(temporal_align_tree, median_max_1, 0, alpha_over, "Foreground")
    link_nodes(temporal_align_tree, input_node, "Frame + 1", alpha_over_ta, "Background")
    link_nodes(temporal_align_tree, median_max_1_ta, 0, alpha_over_ta, "Foreground")

    link_nodes(temporal_align_tree, alpha_over_ta, 0, ta_difference, 6)
    link_nodes(temporal_align_tree, input_node, "Albedo + 1", ta_difference, 7)

    link_nodes(temporal_align_tree, ta_difference, 2 , ta_multiply, 0)
    link_nodes(temporal_align_tree, ta_multiply, 0, ta_dialate, 0)
    link_nodes(temporal_align_tree, ta_dialate, 0, ta_mix, 0)
    link_nodes(temporal_align_tree, alpha_over, 0, ta_mix, 6)
    link_nodes(temporal_align_tree, input_node, "Frame + 1", ta_mix, 7)

    link_nodes(temporal_align_tree, ta_mix, 2, vector_blur, "Image")
    link_nodes(temporal_align_tree, input_node, "Depth + 1", vector_blur, "Depth")
    link_nodes(temporal_align_tree, input_node, "Vector + 1", vector_blur, "Speed")
...
def create_temporal_setup(scene, view_layer_id: int):
    ...
    compositor = get_scene_compositor_tree(scene)
    ...
    # Blender 5.0 removed the dedicated Composite node; the tree's Group Output
    # node is now the final compositor output instead.
    output_node = compositor.nodes.new("NodeGroupOutput")
    if not any(item.in_out == "OUTPUT" for item in compositor.interface.items_tree):
        create_socket(compositor, "Image", "NodeSocketColor", "OUTPUT")
    ...
    output_node = compositor.nodes.new("CompositorNodeOutputFile")
    output_node.directory = path_completed
    ...
    for name in output_names:
        output_node.file_output_items.new("NodeSocketColor", name)
